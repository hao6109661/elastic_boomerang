...
public:
  ...
  // Get arms pointers
  void get_steady_problem_beam_meshes(Mesh*& steady_beam_mesh_first_arm_pt,
                                      Mesh*& steady_beam_mesh_second_arm_pt)
  {
...
public:
  ...
  UnsteadyRigidBodyElement(const double& X0,
                           const double& Y0,
                           const double& Theta_eq,
...
private:
  ...
  /// Pointer to UnsteadyRigidBodyElement that actually contains the rigid body
  /// data
  UnsteadyRigidBodyElement* Rigid_body_element_pt;
  ...
  const std::string& restart_steady_file)
{
  ...
  // ElasticBeamProblem problem(n1, n2);

  Mesh* steady_beam_first_arm_pt;
  Mesh* steady_beam_second_arm_pt;

  SteadyElasticBeamProblem_pt->get_steady_problem_beam_meshes(
    steady_beam_first_arm_pt, steady_beam_second_arm_pt);
  ...
  // Make the UnsteadyRigidBodyElement that stores the parameters for the rigid
  // body motion
  Rigid_body_element_pt =
    new UnsteadyRigidBodyElement(x0, y0, theta_eq, time_stepper_pt());
  ...
  // Pass the pointer of the mesh to the UnsteadyRigidBodyElement class
  // so it can work out the drag and torque on the entire structure
  Vector<SolidMesh*> Beam_mesh_pt(2);
  Beam_mesh_pt[0] = Beam_mesh_first_arm_pt;
  Beam_mesh_pt[1] = Beam_mesh_second_arm_pt;
  ...
  // Loop over nodes in mesh and assign values to positions
  unsigned n_node = Beam_mesh_first_arm_pt->nnode();
  for (unsigned j = 0; j < n_node; j++)
  {
    // Position
    Beam_mesh_first_arm_pt->node_pt(j)->x_gen(0, 0) =
      steady_beam_first_arm_pt->node_pt(j)->x_gen(0, 0);
    Beam_mesh_first_arm_pt->node_pt(j)->x_gen(0, 1) =
      steady_beam_first_arm_pt->node_pt(j)->x_gen(0, 1);

    // Slope
    Beam_mesh_first_arm_pt->node_pt(j)->x_gen(1, 0) =
      steady_beam_first_arm_pt->node_pt(j)->x_gen(1, 0);
    Beam_mesh_first_arm_pt->node_pt(j)->x_gen(1, 1) =
      steady_beam_first_arm_pt->node_pt(j)->x_gen(1, 1);
  }
  ...
  for (unsigned j = 0; j < n_node; j++)
  {
    ...
    Beam_mesh_second_arm_pt->node_pt(j)->x_gen(0, 0) =
      steady_beam_second_arm_pt->node_pt(j)->x_gen(0, 0);
    Beam_mesh_second_arm_pt->node_pt(j)->x_gen(0, 1) =
      steady_beam_second_arm_pt->node_pt(j)->x_gen(0, 1);

    // Slope
    Beam_mesh_second_arm_pt->node_pt(j)->x_gen(1, 0) =
      steady_beam_second_arm_pt->node_pt(j)->x_gen(1, 0);
    Beam_mesh_second_arm_pt->node_pt(j)->x_gen(1, 1) =
      steady_beam_second_arm_pt->node_pt(j)->x_gen(1, 1);
  }
  ...
  for (unsigned e = 0; e < n_element; e++)
  {
    ...
    // Pass the pointer of UnsteadyRigidBodyElement to the each element
    // so we can work out the rigid body motion
    elem_pt->set_pointer_to_rigid_body_element(Rigid_body_element_pt);
