...
file4 = sprintf("RESLT_q_%.2f_alpha_%.3fpi_ds_10-3_restart/elastic_beam_I_theta_q_%.2f_alpha_%.3fpi_initial_%.2f.dat", q, alpha/pi, q, alpha/pi, initial1)
file2 = sprintf("RESLT_q_%.2f_alpha_%.3fpi_ds_10-4_normal/elastic_beam_I_theta_q_%.2f_alpha_%.3fpi_initial_%.2f.dat", q, alpha/pi, q, alpha/pi, initial1)
file3 = sprintf("RESLT_q_%.2f_alpha_%.3fpi_I_increment/elastic_beam_I_theta_q_%.2f_alpha_%.3fpi_initial_%.2f.dat", q, alpha/pi, q, alpha/pi, initial1)
file1 = sprintf("RESLT_q_%.2f_alpha_%.3fpi_arclength_after_I_increment/elastic_beam_I_theta_q_%.2f_alpha_%.3fpi_initial_%.2f.dat", q, alpha/pi, q, alpha/pi, initial1)

# Got the max value of I from the general file 
max_value = system(sprintf("awk '{if(NR==1 || $1 > max) max=$1} END {print max}' \"%s\"", file4))
...
# Produce a tempprary file for the restart inner closed loop solution
# Change its step number to float type to identify its result directory
set table 'filtered_data3.dat'
plot file3 using 1:3:(abs($1 - target) <= epsilon ? ($9)+0.01 : "#"):(abs($1 - target) <= epsilon ? $8 : "#") with table
unset table

# Produce a tempprary file for the restart inner closed loop solution
# Change its step number to float type to identify its result directory
set table 'filtered_data4.dat'
plot file4 using 1:3:(abs($1 - target) <= epsilon ? sprintf("%.3f", $9 + 0.001) : "#"):(abs($1 - target) <= epsilon ? $8 : "#") with table
unset table
...
suffix_list_str = system("awk '$3 ~ /^[0-9]+(\\.[0-9]+)?$/{print $3}' filtered_data1.dat filtered_data2.dat filtered_data3.dat filtered_data4.dat")
...
# Note that here this is in the type of string
theta_list_str = system("awk '$3 ~ /^[0-9]+(\\.[0-9]+)?$/{print $2}' filtered_data1.dat filtered_data2.dat filtered_data3.dat filtered_data4.dat")
...
# Note that here this is in the type of string
I_list_str = system("awk '$3 ~ /^[0-9]+(\\.[0-9]+)?$/{print $1}' filtered_data1.dat filtered_data2.dat filtered_data3.dat filtered_data4.dat")
...
plot file1 using 1:(strcol(3) eq "#" ? 1/0 : $3) with lines linewidth 2 lc rgb "red" title sprintf('q=%.2f, {/symbol a}=%.3fπ', q, alpha/pi), \
     file2 using 1:3 every ::0::0 with points pointsize 1.2 pointtype 7 lc rgb "black" title "I=0" ,\
     file2 every ::(num_lines-1)::(num_lines-1) using 1:3 with points pointsize 1.2 pointtype 7 lc rgb "black" title "",\
     file2 using 1:(strcol(3) eq "#" ? 1/0 : $3) with lines linewidth 2 lc rgb "red" title "", \
     file3 using 1:(strcol(3) eq "#" ? 1/0 : $3) with lines linewidth 2 lc rgb "red" title "", \
     file4 using 1:(strcol(3) eq "#" ? 1/0 : $3) with lines linewidth 2 lc rgb "red" title "", \
...
do for [i=0:counter_branch] {
  ...
  float_rounded1 = system(sprintf("awk '$1 ~ /^[0-9]*\\.[0-9]$/{print int($1)}' %s", filename))
  float_rounded2 = system(sprintf("awk '$1 ~ /^[0-9]*\\.[0-9][0-9]$/{print int($1)}' %s", filename))
  float_rounded3 = system(sprintf("awk '$1 ~ /^[0-9]*\\.[0-9][0-9][0-9]$/{print int($1)}' %s", filename))
  #print float_rounded1
  #print float_rounded3

  # Assemble the file name of outer solutions
  do for [suffix in float_rounded3] {
     # Create file names using suffix and add them to filelist
     filename_first = sprintf("RESLT_q_%.2f_alpha_%.3fpi_ds_10-3_restart/beam_first_arm_initial_%.2f_%s.dat", q, alpha/pi, initial1, suffix)
     filelist[i+1] = filelist[i+1] . "\"" . filename_first . "\" "
     filename_second = sprintf("RESLT_q_%.2f_alpha_%.3fpi_ds_10-3_restart/beam_second_arm_initial_%.2f_%s.dat", q, alpha/pi, initial1, suffix)
  ...
  do for [suffix in float_rounded1] {
     # Create file names using suffix and add them to filelist
      filename_first = sprintf("RESLT_q_%.2f_alpha_%.3fpi_ds_10-4_normal/beam_first_arm_initial_%.2f_%s.dat", q, alpha/pi, initial1, suffix)
      filelist[i+1] = filelist[i+1] . "\"" . filename_first . "\" "
      filename_second = sprintf("RESLT_q_%.2f_alpha_%.3fpi_ds_10-4_normal/beam_second_arm_initial_%.2f_%s.dat", q, alpha/pi, initial1, suffix)
      filelist[i+counter_branch+2] = filelist[i+counter_branch+2] . "\"" . filename_second . "\" "}
      
  # Assemble the file name of inner closed loop solutions  
  do for [suffix in float_rounded2] {
     # Create file names using suffix and add them to filelist
      filename_first = sprintf("RESLT_q_%.2f_alpha_%.3fpi_I_increment/beam_first_arm_initial_%.2f_%s.dat", q, alpha/pi, initial1, suffix)
      filelist[i+1] = filelist[i+1] . "\"" . filename_first . "\" "
      filename_second = sprintf("RESLT_q_%.2f_alpha_%.3fpi_I_increment/beam_second_arm_initial_%.2f_%s.dat", q, alpha/pi, initial1, suffix)
      filelist[i+counter_branch+2] = filelist[i+counter_branch+2] . "\"" . filename_second . "\" "}
      
  # Assemble the file name of inner closed loop solutions  
  do for [suffix in integers] {
     # Create file names using suffix and add them to filelist
      filename_first = sprintf("RESLT_q_%.2f_alpha_%.3fpi_arclength_after_I_increment/beam_first_arm_initial_%.2f_%s.dat", q, alpha/pi, initial1, suffix)
      filelist[i+1] = filelist[i+1] . "\"" . filename_first . "\" "
      filename_second = sprintf("RESLT_q_%.2f_alpha_%.3fpi_arclength_after_I_increment/beam_second_arm_initial_%.2f_%s.dat", q, alpha/pi, initial1, suffix)
      filelist[i+counter_branch+2] = filelist[i+counter_branch+2] . "\"" . filename_second . "\" "}
}
...
system("rm filtered_data3.dat")
system("rm filtered_data4.dat")
do for [i=0:counter_branch] {
