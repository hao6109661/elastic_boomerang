...
initial1=0.00
initial2=-0.10
initial4=initial1+3.14
initial3=initial2-3.14
...
# Set the output file 
set output sprintf("combine_elastic_beam_I_theta_q_%.2f_alpha_%.3fpi_initial_%.2f_%d.png", q, alpha/pi, initial3, i)
...
I_value = real(system(sprintf("awk 'NR==%d+1{print $1}' \"%s\"", i, file5)))
...
plot file1 using 1:(strcol(3) eq "#" ? 1/0 : $3) with points pointsize 0.5 pointtype 7 lc rgb "red" title sprintf('q=%.2f, {/symbol a}=%.3fπ', q, alpha/pi), \
     ...
     file5 using 1:(strcol(3) eq "#" ? 1/0 : $3) every ::i::i with points pointsize 1.2 pointtype 7 lc rgb "blue" title "point considered with iteration", \
     file5 using 1:3:(stringcolumn(8)) every ::i::i with labels offset label_offset_x,label_offset_y font ",label_size" tc rgb "blue" title ""
...
file3 = sprintf("beam_first_arm_initial_%.2f_%d.dat",initial3, i)
file4 = sprintf("beam_second_arm_initial_%.2f_%d.dat",initial3, i)
